...
export function Time(props: Props) {
  ...
  const time = createMemo(() =>
    formatTime(
      dayjs,
      props,
      // Only relative formats depend on the current time; the parameter is
      // ignored for the rest, so don't subscribe them to the clock.
      props.format === "relative" ? datePerMinute() : undefined,
    ),
  );
